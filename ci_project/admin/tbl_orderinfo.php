...
//
class ctbl_order extends cTable {
	var $order_id;
	var $cus_id;
	var $order_detail;
	...
	function __construct() {
		...
		// order_id
		$this->order_id = new cField('tbl_order', 'tbl_order', 'x_order_id', 'order_id', '`order_id`', '`order_id`', 3, -1, FALSE, '`order_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// cus_id
		$this->cus_id = new cField('tbl_order', 'tbl_order', 'x_cus_id', 'cus_id', '`cus_id`', '`cus_id`', 3, -1, FALSE, '`cus_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// order_detail
		$this->order_detail = new cField('tbl_order', 'tbl_order', 'x_order_detail', 'order_detail', '`order_detail`', '`order_detail`', 200, -1, FALSE, '`order_detail`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->order_detail->Sortable = TRUE; // Allow sort
		$this->fields['order_detail'] = &$this->order_detail;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tbl_order`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tbl_orderview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tbl_orderview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tbl_orderadd.php?" . $this->UrlParm($parm);
		else
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tbl_orderedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tbl_orderadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tbl_orderdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->order_detail->setDbValue($rs->fields('order_detail'));
	}
	...
	function RenderEditRow() {
		...
		$this->order_detail->EditValue = $this->order_detail->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->order_detail->Exportable) $Doc->ExportCaption($this->order_detail);
				} else {
					if ($this->order_id->Exportable) $Doc->ExportCaption($this->order_id);
					if ($this->cus_id->Exportable) $Doc->ExportCaption($this->cus_id);
					if ($this->order_detail->Exportable) $Doc->ExportCaption($this->order_detail);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->order_detail->Exportable) $Doc->ExportField($this->order_detail);
					} else {
						if ($this->order_id->Exportable) $Doc->ExportField($this->order_id);
						if ($this->cus_id->Exportable) $Doc->ExportField($this->cus_id);
						if ($this->order_detail->Exportable) $Doc->ExportField($this->order_detail);
