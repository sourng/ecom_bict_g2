  ...
  <div class="table-responsive">
   ...
   foreach($product as $row)
   {
    ...
    <div class="col-md-4" style="padding:16px; background-color:#f1f1f1; border:1px solid #ccc; margin-bottom:16px; height:400px" align="center">
     <img src="'.base_url().'upload/product/'.$row->pro_feature.'" class="img-thumbnail" /><br />
     <h4>'.$row->pro_name.'</h4>
     <h3 class="text-danger">$'.$row->price.'</h3>
     <input type="text" name="quantity" class="form-control quantity" id="'.$row->pro_id.'" /><br />
     <button type="button" name="add_cart" class="btn btn-success add_cart" data-productname="'.$row->pro_name.'" data-price="'.$row->price.'" data-productid="'.$row->pro_id.'" />Add to Cart</button>
    </div>
    ';
   }
   ?>
